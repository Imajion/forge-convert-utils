...
const big_json_1 = require("big-json");
...
class Writer {
    ...
    constructor(options = {}) {
        ...
        this.options = {
            ...
            centerOnTranslation: (0, util_1.isNullOrUndefined)(options.centerOnTranslation) ? [] : options.centerOnTranslation,
     ...
     */
    async write(imf, outputDir, metadataFrontVector) {
    ...
    serializeManifest(manifest, outputPath) {
        return new Promise(res => {
            const stringifyStream = (0, big_json_1.createStringifyStream)({
                body: manifest,
            });
    ...
    createScene(imf, metadataFrontVector) {
        ...
        let totalBounds = new math_1.Box3();
        ...
        for (let i = 0, len = imf.getNodeCount(); i < len; i++) {
            ...
            if (!(0, util_1.isUndefined)(node.mesh)) {
                ...
                if (fragment.bbox) {
                    ...
                    const nodeBounds = new math_1.Box3(new math_1.Vec3(min.x, min.y, min.z), new math_1.Vec3(max.x, max.y, max.z));
        ...
        if (this.options.center) {
            if (!totalBounds.empty) {
                ...
                const calculatedCenter = [-center.x * scale, -center.y * scale, -center.z * scale];
            ...
            if (metadata["world bounding box"]) {
                ...
                if (boundsMin && boundsMax) {
                    ...
                    const calculatedCenter = [-translation[0] * scale, -translation[1] * scale, -translation[2] * scale];
