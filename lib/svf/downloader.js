...
class Downloader {
    constructor(authenticationProvider) {
        ...
        this.modelDerivativeClient = new model_derivative_1.ModelDerivativeClient();
    }
    download(urn, options) {
        ...
        return {
            ready: this._download(urn, context, options === null || options === void 0 ? void 0 : options.region),
            cancel: () => { context.cancelled = true; }
        };
    }
    async _downloadDerivative(urn, derivativeUrn, region) {
        try {
    ...
    async _download(urn, context, region) {
        ...
        for (const derivative of derivatives) {
            ...
            const svf = await this._downloadDerivative(urn, encodeURI(derivative.urn), region);
            fse.writeFileSync(path.join(guidDir, 'output.svf'), new Uint8Array(svf));
            const reader = await __1.SvfReader.FromDerivativeService(urn, guid, this.authenticationProvider, region);
