...
class Reader {
    ...
    static async FromDerivativeService(urn, guid, auth, host, region) {
        urn = urn.replace(/=/g, '');
        const modelDerivativeClient = new forge_server_utils_sitelink_1.ModelDerivativeClient(auth, host, region);
        const helper = new forge_server_utils_sitelink_1.ManifestHelper(await modelDerivativeClient.getManifest(urn));
